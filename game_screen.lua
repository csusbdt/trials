...
local function draw_next()
	...
	next_texture:draw(design_width - 66, main_y + 132)
end
...
local function draw_name()
	...
	local x = 385 - texture.w / 2
	local y = 353 - texture.h / 2
	texture:draw(x, y)
end
...
local function draw_date()
	...
	local date_y = bar_y + date_texture.h / 2 + 3 -- adjust down a little
...
local function draw_menu_button()
	local x = design_width - 140
	local texture = date_font:text("MENU", black)
	local y = design_height - 31 - texture.h / 2
	menu_button = buttons.create_from_texture(texture, x, y)
	menu_button:draw()
end

local function draw_hide_button()
	local texture = textures.image('ui/UI-mobile-diamond-black.png')
	local y = design_height - 31 - texture.h / 2
	hide_button = buttons.create_from_texture(texture, design_width - 30, y)
	hide_button:draw()
end
